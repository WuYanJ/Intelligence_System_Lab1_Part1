    ...
    for iter in range(iteration):
        ...
        for batch in range(batch_amount):
            ...
            for i in range(len(M) - 1):
                DW.append(mat(np.zeros((M[i], M[i + 1]))))  # 每一个列表项都是784x60,60x12的矩阵
                Db.append(mat(np.zeros((M[i + 1], 1))))
            for i in sampleList[start:start+batch_size:1]:
            ...
            for m in range(len(W)):  # 对于每一个权重矩阵
                W[m] -= (newRate / batch_size) * DW[m]
                # print("W[1]\n", W[1])
                b[m] -= (newRate / batch_size) * Db[m]
        print(loss.tolist()[0])
...
if __name__ == "__main__":
    ...
    # store(W, './Params/weights_1.txt')
    # store(b, './Params/biases_1.txt')
    ...
    W = readParam('./Params/weights_1.txt')
    b = readParam('./Params/biases_1.txt')
    test(M, W, b)