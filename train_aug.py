...
def backward(M, W, b, net, out, y, E, label, rate, lmd=0.00013):
    """
    后向推导函数，返回更新后的W, b参数
    :param M: 层数向量
    :param W: 权值矩阵
...
def training(M, W, b, iteration=5):
    ...
    title = 'lr=' + str(rate) + ' M=' + str(M) + ' iter=' + str(iteration) + ' no batch_size CE lmd=0.00013'
    plot.title(title)
    plot.show()
            # print("第%d个样本训练！"%i)
...
if __name__ == "__main__":
    ...
    # store(W, 'Params/weights_aug.txt')
    # store(b, 'Params/biases_aug.txt')

    W = readParam('Params/weights_aug.txt')
    b = readParam('Params/biases_aug.txt')
    test(M, W, b)