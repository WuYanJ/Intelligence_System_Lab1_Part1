...
def training(M, W, b, iteration):
    ...
    title = 'batch se lr=' + str(rate) + ' M=' + str(M) + ' iter=' + str(iteration) + ' batch_size=' + str(batch_size) + ' SE'
    plot.title(title)
    plot.show()
    return W, b
...
if __name__ == "__main__":
    M = [784, 60, 12]
    # W, b = wbInit(M)
    # W, b = training(M, W, b, 300)
    # store(W, 'weights_batch_se.txt')
    # store(b, 'biases_batch_se.txt')
    W = readParam('Params/weights_batch_se.txt')
    b = readParam('Params/biases_batch_se.txt')
    # print(W)
    # W = readParam('weights0.85MSE.txt')
    # b = readParam('biases0.85MSE.txt')
    # W, b = training(M, W, b, 50)
    # store(W, 'weights-batch0.77.txt')
    # store(b, 'biases-batch0.77.txt')
    test(M, W, b)