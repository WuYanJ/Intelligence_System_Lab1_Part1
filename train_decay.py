...
def training(M, W, b, iteration=5, lmd=0.05):
    """
    训练模型,输出结果
    :param M:神经元数量向量
    :param W: 初始化的权值矩阵
    ...
    for iter in range(iteration):
        ...
        layer.reverse()
        for m in layer:
            W[m - 1] -= rate * lmd * W[m - 1]
    plot.figure()
    plot.plot(xList, lossList, 'o')
    plot.plot(xList, testLossList, 'ro')
    title = 'lr=' + str(rate) + ' M=' + str(M) + ' iter=' + str(iteration) + ' no batch_size CE decay lmd=0.05'
    plot.title(title)
    plot.show()
            # print("第%d个样本训练！"%i)
...
if __name__ == "__main__":
    M = [784, 60, 12]
    W, b = wbInit(M)
    W, b = training(M, W, b, 30)
    store(W, 'Params/weights_decay.txt')
    store(b, 'Params/biases_decay.txt')

    # W = readParam('Params/weights_decay.txt')
    # b = readParam('Params/biases_decay.txt')
    # test(M, W, b)