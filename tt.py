...
def training(M, W, b, iteration=5):
    ...
    for iter in range(iteration):
        ...
        for i in range(12 * 20):
            ...
            net, out, y, E = forward(M, W, b, im_matrix, lab_matrix)
            lossTest += E
        testLossList.append(lossTest * 30)
    plot.figure()
...
if __name__ == "__main__":
    M = [784, 60, 12]
    W, b = wbInit(M)
    W, b = training(M, W, b, 80)
    store(W, 'Params/weightstt.txt')
    store(b, 'Params/biasestt.txt')

    # W = readParam('Params/weightstt.txt')
    # b = readParam('Params/biasestt.txt')
    # test(M, W, b)